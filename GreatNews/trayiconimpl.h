...
public:	
	...
	{
		WM_TRAYICON = ::RegisterWindowMessage(_T("GN_TRAYICON"));
	}
	...
	bool InstallIcon(LPCTSTR lpszToolTip, HICON hIcon, UINT nID, bool bReinstall=false)
	{
		...
		if(lpszToolTip)
		{
			...
			StringCchCopy(m_nid.szTip, sizeof(m_nid.szTip)/sizeof(TCHAR), lpszToolTip);
		}
		// Install
		if(bReinstall || !m_bInstalled)
	...
	bool SetTooltipText(LPCTSTR pszTooltipText)
	{
		...
		// Fill the structure
		m_nid.uFlags = NIF_TIP;
		StringCchCopy(m_nid.szTip, sizeof(m_nid.szTip)/sizeof(TCHAR), pszTooltipText);
		// Set
		if(Shell_NotifyIcon(NIM_MODIFY, &m_nid))
		{
			m_tooltipText = pszTooltipText;
			return true;
		}
		else
			return false;
	...
	BOOL ShowBalloonTip(LPCTSTR szMsg, LPCTSTR szTitle, UINT uTimeout=10000, DWORD dwInfoFlags=NIIF_INFO)
	{
		if (!m_bInstalled)
			return false;
		
		m_nid.uFlags = NIF_INFO;
		m_nid.uTimeout = uTimeout | NOTIFYICON_VERSION;
		m_nid.dwInfoFlags = dwInfoFlags;
		StringCchCopy(m_nid.szInfo, sizeof(m_nid.szInfo)/sizeof(TCHAR), szMsg ? szMsg : _T(""));
		StringCchCopy(m_nid.szInfoTitle, sizeof(m_nid.szInfoTitle)/sizeof(TCHAR), szTitle ? szTitle : _T(""));
