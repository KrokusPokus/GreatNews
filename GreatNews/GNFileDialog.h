...
class CGNFileDialog : public CFileDialogImpl<CGNFileDialog>
{
public:
	CGNFileDialog(BOOL bOpenFileDialog, // TRUE for FileOpen, FALSE for FileSaveAs
		...
		: CFileDialogImpl<CGNFileDialog>(bOpenFileDialog, lpszDefExt, lpszFileName, dwFlags, lpszFilter, hWndParent)
	{ 
		...
		{
			size_t len = _tcslen(lpszFilter)+1;
			m_szFilter = new TCHAR[len];
			StringCchCopy(m_szFilter, len, lpszFilter);
