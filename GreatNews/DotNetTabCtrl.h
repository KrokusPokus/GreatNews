...
public:
	...
	void DrawItem_ImageAndText(DWORD /*dwStyle*/, LPNMCTCCUSTOMDRAW lpNMCustomDraw, int nIconVerticalCenter, RECT& rcTab, RECT& rcText)
	{
		...
		if (rcText.left + m_nMinWidthToDisplayText < rcText.right)
		{
			...
			dc.DrawText(sText, sText.GetLength(), &rcText, DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_WORD_ELLIPSIS | DT_NOPREFIX);
		}
	...
	void UpdateLayout_Default(RECT rcTabItemArea)
	{
		...
		for( size_t i=0; i<nCount; ++i )
		{
			...
			if(pItem->UsingText())
			{
				RECT rcText = {0};
				_CSTRING_NS::CString sText = pItem->GetText();
				dc.DrawText(sText, sText.GetLength(), &rcText, DT_SINGLELINE | DT_NOPREFIX | DT_CALCRECT);
				rcText.right = min(rcText.right, rcText.left+200);
...
public:
	...
	DECLARE_WND_CLASS_EX(_T("GN_TabCtrl"), CS_DBLCLKS, COLOR_WINDOW)
