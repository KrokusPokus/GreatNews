...
public:
	...
	//<summary>Cast attribute value as tstring. If not found, return empty.</summary>
	//<returns>The tstring attribute value, or empty.</returns>
	...
	//<summary>Set attribute to tstring.</summary>
	//<param name="rhs">Value tstring to set.</param>
	//<returns>Reference to xml_attribute.</returns>
	xml_attribute& operator=(const tstring& rhs){ value(rhs.c_str()); return *this; }
	...
	//<summary>Right-shift attribute value to tstring.</summary>
	//<param name="rhs">Reference to tstring to set.</param>
	//<returns>Reference to xml_attribute.</returns>
	xml_attribute& operator>>(tstring& rhs)
	{
#ifdef PUGOPT_NONSEG
		rhs.clear();
...
public:
	...
	//bool has_name(const tstring& name) { return has_name(name.c_str()); } //Node is named 'name'.
	bool has_attribute(const tstring& name) { return has_attribute(name.c_str()); } //Node has an attribute named 'name'.
#ifdef PUGOPT_NONSEG
...
public:
	...
	xml_attribute attribute(const tstring& name){ return attribute(name.c_str()); }
	//<summary>Access or create the attribute having 'name'.</summary>
	//<param name="name">Name of attribute to access/create.</param>
	//<returns>Reference to xml_attribute wrapper.</returns>
...
#ifdef PUGOPT_NONSEG
						min(valuelen,(unsigned)node->value_size);
#else
						min(valuelen,(unsigned)_tcslen(node->value));
#endif
...
public:
	...
	xml_attribute_struct* mapto_attribute_ptr(const tstring& name){ return mapto_attribute_ptr(name.c_str()); }
	...
	xml_node_struct* mapto_child_ptr(const tstring& name){ return mapto_child_ptr(name.c_str()); }
	...
	//<returns>Index of child, or -1 if not found.</returns>
	//<remarks>Implement your own hash table if you have a great many children.</remarks>
	int mapto_child_idx(const tstring& name){ return mapto_child_idx(name.c_str()); }
...
public:
	...
	void all_elements_by_name(const tstring& name,pointer_array& found){ all_elements_by_name(name.c_str(),found); }
	...
	xml_node first_element_by_name(const tstring& name){ return first_element_by_name(name.c_str()); }
	...
	//<remarks>xml_node may be invalid if not found; test with 'empty'.</remarks>
	xml_node first_element_by_value(const tstring& name,const tstring& value){ return first_element_by_value(name.c_str(),value.c_str()); }
	...
	//<returns>Valid xml_node if such element named 'name' is found.</returns>
	//<remarks>xml_node may be invalid if not found; test with 'empty'.</remarks>
	xml_node first_element_by_attribute(const tstring& name,const tstring& attr_name,const tstring& attr_value){ return first_element_by_attribute(name.c_str(),attr_name.c_str(),attr_value.c_str()); }
	...
	//<remarks>If matching co-node was found, '_root' points thereto.</remarks>
	bool moveto_first_sibling(const tstring& name){ return moveto_first_sibling(name.c_str()); }
	...
	//<remarks>If matching sub-node was found, '_root' points thereto.</remarks>
	bool moveto_child(const tstring& name){ return moveto_child(name.c_str()); }
	...
	bool moveto_next_sibling(const tstring& name){ return moveto_next_sibling(name.c_str()); }
	...
	tstring path(const TCHAR* delimiter = _T("/"))
	{
		...
		strcatgrow(&temp,path); //Append current path.
		free(path); //Free the old path.
		tstring returns = temp; //Set path as new string.
		free(temp);
	...
	// xml_node first_element_by_path(const tstring& path,const tstring& delimiter = _T("/")){ return first_element_by_path(path.c_str(),delimiter.c_str()); }
...
public:
	...
	//<returns>Success.</returns>
	bool name(const tstring& new_name){ return name(new_name.c_str()); }
	...
	//<returns>Success.</returns>
	bool value(const tstring& new_value){ return value(new_value.c_str()); }
	...
	//<returns>Success.</returns>
	bool remove_attribute(const tstring& name){ return remove_attribute(name.c_str()); }
	...
	xml_attribute append_attribute(const tstring& name,const tstring& value){ return append_attribute(name.c_str(),value.c_str()); }
...
public:
	...
	//
	// My extension: we need this one to skip DTD, PI etc
	//
	xml_node root_element()
	{
		xml_node doc = document();
		if(doc.has_child_nodes())
		{
			for(unsigned int i=0;i<doc.children();++i)
			{
				int nodeType = doc.child(i).type();
